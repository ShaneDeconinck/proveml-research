...
\caption{What the reader sees. The same marked-up text after verification: a wrong revenue figure struck through in red, a claim against an entity the store does not know in amber, and a judgment whose registered condition does not hold in dashed red. Nothing here required reading the markup, and nothing required a model. Section~\ref{sec:rendering} covers the rendering states; Figure~\ref{fig:render-audit} shows a companion example in audit mode, with the fact-store path behind each claim made visible.}
