...
\vspace{0.4em}
\textbf{What it requires, costs and cannot do.} ProveML applies where the claims are backed by an addressable structured source: without a fact store there is nothing to resolve against. Marked-up responses ran 49--79\% longer in characters than the same text without markup. Exact string equality forbids rounded prose --- a verified sentence must say \texttt{391035000000}, not ``\$391 billion'' --- derived values with no path in the store cannot be bound, and the threshold construct, while specified and tested, was never produced by a model in our runs.
...
\toprule
\textbf{Benchmark} & \textbf{Model} & \textbf{First pass} & \textbf{After one correction} & \textbf{Fully verified} & \textbf{Coverage} \\
\midrule
