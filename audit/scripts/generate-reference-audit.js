...
const referenceRecords = bibEntries.map((entry) => {
  ...
  // Snapshots are kept as .html where small, and as extracted .txt where the
  // archived page carried megabytes of assets. Either counts as a snapshot.
  const snapshotCandidates = ['html', 'txt'].map((ext) =>
    path.join('references', 'raw', `${entry.key}.${ext}`));
  const localSnapshotPath = snapshotCandidates.find((rel) => {
