...
\begin{enumerate}
    ...
    \item \textbf{Qwen 3B, Haiku and Qwen 7B} sit together at 88--89\% first-pass verification and stay there. At three runs each these three are not separable: their per-run values are 88/88/90, 85/88/90 and 83/91/92, so the ranges overlap almost entirely and the ordering in Table~\ref{tab:capability} carries no signal. We report the means for completeness and draw no conclusion from the differences between them. What the three do share is the shape of the residue: addressability errors account for roughly three quarters to four fifths of it. They differ in how much they say rather than how reliably: Qwen 7B converges on the most queries (23 of 28) while producing the fewest claims ($\approx$100 per run against Haiku's $\approx$172), and even that gap is about one between-run standard deviation, so a shorter answer with less to get wrong is the more parsimonious reading.
\end{enumerate}
...
For the three larger models the effect is negligible and does not point one way: Haiku +1pp, Qwen 3B $\pm$0, Qwen 7B $-$2pp, all well inside run-to-run variation. Coverage moves more than rate does for the two Qwen models (3B 72.5\% $\rightarrow$ 87.9\%, 7B 60.5\% $\rightarrow$ 69.7\%) while Haiku moves the other way (91.9\% $\rightarrow$ 87.7\%), which is a reminder that the two metrics are not measuring the same thing. Both language conditions were stored under the same 2000-character cap, and it bound rarely and roughly evenly (Phi-3 3 of 84 Dutch responses, 7 of 83 English; no other model reached it), so the coverage comparison is not an artifact of truncation.

\paragraph{What this revises.} Two conclusions follow. First, the ordering between models is not an artifact of language: even on English prompts Phi-3 remains far below the other three (60\% against 87--89\%), so translating the request does not turn a 3.8B model into a reliable one. Second, the cross-domain gap of Section~\ref{sec:finance} for this model (38\% education against 92\% finance) decomposes into roughly 22pp attributable to prompt language and the remainder to benchmark structure --- a claim that context complexity alone explains that gap would have been wrong, and so would a claim that language alone does. Non-English deployment is thus a first-class variable for small models under ProveML, and it shows up less as a lower score than as a model that intermittently produces nothing: the same schema, verifier, and thresholds yield markedly different reliability depending on the language the request arrives in.
