...
\vspace{0.4em}
Across four models (3.8B to API-scale, 3 runs each) first-pass verification ranges from 15\% to 93\%, and residual failures are dominated by addressability errors rather than wrong values. A cross-domain evaluation on both a synthetic educational dataset and real SEC EDGAR financial data shows that this spread tracks the shape of the request rather than model size: on the compact benchmark all four models, including the 3.8B one, produce reliably verifiable markup. A language ablation localizes a large part of that difference: translating the educational prompts to English moves the 3.8B model by 42 percentage points, while the other three shift by at most 4, so non-English deployment is a first-class variable for small models and a minor one above that. A markup coverage audit shows that verification rate alone overstates the verified share of a report's numeric content, and a baseline against SymGen's substitution mechanism shows both approaches failing on addressability but failing differently: substitution leaves \texttt{undefined} in the text where ProveML leaves a flagged claim. We provide the syntax specification, verification algorithm, reference implementation, and an evaluation spanning two domains, four models, a baseline and two ablations.
\end{abstract}
...
The regulatory landscape compounds the problem. The EU AI Act \citep{euaiact} classifies AI systems used for consequential decisions as ``high-risk'' (Annex III), requiring transparency, human oversight, and risk management. The 2026 Digital Omnibus \citep{omnibus2026} deferred those stand-alone high-risk obligations to 2 December 2027 (and to 2 August 2028 for AI embedded in regulated products) while leaving the Article 50 transparency duties in force from 2 August 2026 --- a deferral that buys providers and deployers time to build verifiable-claim infrastructure rather than removing the need for it. Yet to our knowledge, \textbf{no existing framework combines inline claim markup, deterministic mismatch detection against structured data, and composable threshold inference} in a single system. We screened the titles and abstracts of the 4{,}617 papers in the ACL 2026 main, short, findings and industry volumes against concept patterns for claim-level attribution, structured-data faithfulness, symbolic or deterministic verification, provenance and markup schemes, then read the abstracts of the resulting candidates. That sweep found systems that bind claims to evidence contracts \citep{gavel}, emit provenance triples \citep{genprove}, or maintain typed key-value state with non-probabilistic admission \citep{toolgate}, but none that extends an authoring format with inline claim markup, and none that constrains qualitative wording through a declared threshold vocabulary. Title-and-abstract screening can miss a mechanism described only in a paper's body, so we state this as the result of a search rather than as an exhaustive negative.
...
\begin{enumerate}
    \item \textbf{A specification} of the ProveML markup language, including syntax, verification algorithm, and grammar (Sections 3--5 and Appendix~\ref{app:grammar}).
    ...
    \item \textbf{An empirical evaluation} across four models (3.8B to API-scale) and two domains, with a baseline against SymGen's substitution mechanism, two ablations (context slicing, prompt language) and a markup coverage audit. Most residual failures are addressability errors rather than value mismatches, and prompt language moves the smallest model by 42 percentage points (Section 7).
    \item \textbf{A reference implementation} with 186 tests (plugin, grammar, paper-example, CLI, and detection checks), tested against both a synthetic educational dataset and real SEC EDGAR financial data (Section 9).
\end{enumerate}
...
Four outcomes: \textit{verified} (match), \textit{mismatch} (value differs), \textit{unverifiable} (field not found), \textit{no context} (no entity precedes the fact). Section 4 names the implementation's finer-grained statuses; Section 6 maps them onto rendering states.
...
\caption{Verification errors. A wrong revenue figure is shown in red with strikethrough; an unknown entity (Alphabet) renders in amber; a failed inference is shown in red dashed underline (this demo store's Acme balance is positive, so the negative-balance inference fails).}
...
The evaluation has four parts: an error-detection study of the verifier itself, a four-model study on an educational benchmark and its cross-domain counterpart on SEC EDGAR filings, two ablations that separate prompt language and context selection from the rest, and a baseline against SymGen's substitution mechanism together with an audit of how much of each response is bound at all. Every run reported below is preserved in the artifact repository, and every table regenerates from those files.
...
\midrule
Phi-3 Mini (3.8B) & 15\% $\pm$ 25 & 15\% $\pm$ 25 & 1.7/28 & 13 (65\%) & 3 (15\%) \\
Qwen 2.5 3B & 86\% $\pm$ 1.7 & 87\% $\pm$ 0.6 & 19/28 & 19 (83\%) & 4 (17\%) \\
Claude Haiku & 87\% $\pm$ 3.5 & 88\% $\pm$ 1.5 & 17.7/28 & 21 (88\%) & 3 (13\%) \\
Qwen 2.5 7B & 93\% $\pm$ 4.4 & 93\% $\pm$ 4.4 & 25/28 & 6 (75\%) & 2 (25\%) \\
\bottomrule
\end{tabular}
...
\paragraph{What the spread looks like.} Across 3 runs per model, three patterns are visible. We describe them rather than name them: with one model per pattern, any taxonomy would outrun the evidence.

\begin{enumerate}
    \item \textbf{Phi-3 (3.8B)}: the model is unstable across runs (15\% $\pm$ 25, range 0--44\%), producing usable markup only intermittently. Addressability and context errors dominate. The loop provides no reliable benefit. An earlier informal run appeared to show 50\% verification; repeated evaluation under the final protocol (range 0--44\%) reveals this was not stable.
    \item \textbf{Qwen 3B and Haiku}: models produce markup reliably with 86--87\% first-pass verification, stable across runs ($\pm$ 1.7--3.5). Addressability errors dominate (83--88\% of residuals). The loop's average benefit (+1pp) is within run-to-run variation here; Section~\ref{sec:finance} shows the benefit is a model property (Haiku gains consistently, Qwen 7B never).
    \item \textbf{Qwen 7B}: the model produces highly verifiable output on the first pass (93\% $\pm$ 4.4), converging on 25 of 28 queries. ProveML's value shifts from correction to compliance: catching residual errors and providing an audit trail.
\end{enumerate}

\paragraph{Error classes.} Across the three stable models, residual failures are dominated by addressability errors (75--88\%) rather than value mismatches. This suggests that ProveML's main benefit is not only catching wrong numbers, but exposing incorrect or malformed links between generated claims and the underlying data. Some addressability errors cascade: a single wrong entity ID can produce multiple downstream field-not-found errors. Two caveats: models differ in claim volume (education, final claims per run: Qwen 7B $\approx$120, Haiku $\approx$180), so convergence counts are not directly comparable across models; and the class counts are means over runs with small per-run error totals (Phi-3's classification derives from its single markup-producing run).

\paragraph{Unsupported prompts do not induce abstention.} On the prompts whose answers the data cannot support, no capable model ever abstained from markup: across all runs, Qwen 3B, Qwen 7B, and Haiku produced verifiable-but-tangential claims on every unsupported prompt (education: means of 4.2, 2.4, and 8.3 claims per response at 68--92\% verified; finance: 100\% for all three). Phi-3's zero-markup responses on these prompts reflect its general markup failure, not abstention. Claim-level verification is thus orthogonal to abstention: every individual claim can be consistent with the store while the response as a whole answers a question the data cannot answer. This is the concrete case for the exploratory-language rule (Section~\ref{sec:boundary}) and for a dedicated abstention metric.

\paragraph{Context selection as a first-class concern.} A preserved full-context ablation (one run per local model; \texttt{fullctx} artifacts in the repository) makes this failure mode concrete: given the full dataset as context instead of a slice, all three local models --- including Qwen 7B, the top performer under slicing --- produced \textit{zero} ProveML constructs on every completed query (0 claims over 22, 24, and 27 stored queries; the remainder were generation timeouts on the oversized prompts). The failure is not silence: those same responses contain 38, 97, and 200 numeric tokens in plain prose (0\% markup coverage by the measure of Section~\ref{sec:coverage}). Full context does not stop these models from reporting numbers --- it stops them from reporting numbers that can be checked. With sliced context, the 3B model achieved 86\% first-pass verification, stable across runs. This is not a benchmark convenience: context selection (retrieval, filtering, query routing) is standard in production deployments, and ProveML should be evaluated inside that architecture. The finding is that verification and context selection work together: context selection brings models above the capability threshold, and verification ensures the claims they produce are correct. Neither alone is sufficient.
...
\paragraph{Benchmark structure, not model size.} On this compact, structurally clean benchmark (2 entities, 10 fields each, English prompts), all four models entered the verifiable boundary reliably --- including Phi-3 Mini, which scored 92\% $\pm$ 2.5 here versus 15\% $\pm$ 25 on the educational benchmark (95 class offerings, 741 students). The spread seen on the educational benchmark disappears here: even the weakest model produces stable, high-quality markup when the schema is compact, the entities are few, and the prompts are in English. Section~\ref{sec:langablation} shows how much of that is the language.

\paragraph{Loop benefit is a model property, not a benchmark property.} On this benchmark the loop added nothing for three of four models; only Haiku improved (+5pp). Across all three benchmark variants Haiku is the only model that improves consistently (+0.7pp Dutch, +3.7pp English, +5.0pp finance), while Qwen 7B never gains more than +0.7pp and Phi-3 gains only where it has verifiable markup to repair (+3.3pp on English prompts, +0.0pp on Dutch). The loop is therefore selective in a specific sense: it helps models that already produce well-formed markup with repairable addressing errors, and it cannot rescue a model that produces no markup at all. Where the loop adds nothing, ProveML's value is compliance and auditability rather than correction.
...
\midrule
Phi-3 Mini (3.8B) & 15\% $\pm$ 25 & 56\% $\pm$ 8 & 41.6\% & 68.7\% \\
Qwen 2.5 3B & 86\% $\pm$ 1.7 & 90\% $\pm$ 0.6 & 69.8\% & 93.5\% \\
Claude Haiku & 87\% $\pm$ 3.5 & 88\% $\pm$ 2.0 & 87.3\% & 89.4\% \\
Qwen 2.5 7B & 93\% $\pm$ 4.4 & 91\% $\pm$ 1.5 & 69.7\% & 64.0\% \\
\bottomrule
\end{tabular}
...
\paragraph{The language effect is a small-model effect.} Translating the prompts moves Phi-3 Mini from 15\% $\pm$ 25 to 56\% $\pm$ 8 first-pass verification (+42pp), from 1.7 to 11.0 of 28 queries converged, and from 41.6\% to 68.7\% markup coverage --- and it stabilizes the model, cutting the between-run spread from $\pm$25 to $\pm$8. The coverage half of that comparison carries a caveat: Phi-3's Dutch responses hit the storage cap far more often than its English ones (36 of 80 against 7 of 84), so the Dutch cell is measured on more prefixes and part of the coverage gap may be an artifact of that. The verification-rate half is unaffected, since rates were computed at run time on the full responses. For the three larger models the effect is small: Qwen 3B +4pp, Haiku +1pp, Qwen 7B $-$2pp. Only Haiku's and Qwen 7B's shifts fall inside run-to-run variation; Qwen 3B's Dutch and English runs do not overlap (85--88\% against 89--90\%), so its gain is consistent even though it is modest.

\paragraph{What this revises.} Two conclusions follow, and the second corrects a reading we would otherwise have drawn. First, the ordering between models is not an artifact of language: even on English prompts Phi-3 remains far below the other three (56\% vs.\ 88--91\%), and Qwen 7B still converges on 25 of 28 queries. Second, the \textit{magnitude} of Phi-3's educational collapse was substantially a language effect, not a pure context-complexity effect --- so the cross-domain gap of Section~\ref{sec:finance} (15\% vs.\ 92\%) decomposes into roughly 42pp attributable to prompt language and the remainder to benchmark structure. A claim that context complexity alone explains that gap would have been wrong. Non-English deployment is thus a first-class variable for small models under ProveML: the same schema, verifier, and thresholds yield markedly different verifiability depending on the language the request arrives in.

\subsection{Baseline: Substitution versus Verification}
\label{sec:symgen}

\paragraph{Setup.} SymGen \citep{symgen} is the closest published mechanism, and it takes the opposite strategy: the model emits Jinja-like \texttt{\{\{ field \}\}} references into the conditioning data and a parser substitutes each one, so the model never states a value itself. Unresolvable references render as \texttt{undefined}. We reimplemented its Direct strategy against the same fact stores, prompts, context slices and models used above (3 runs per model per benchmark; the Indirect generate-then-rewrite variant is not reimplemented). Only the markup mechanism differs.
...
Education & 75.4\% & 72.6\% & 67.1\% & 229 \\
\bottomrule
\end{tabular}
\caption{SymGen reimplementation against ProveML on identical models, prompts and data (4 models $\times$ 3 runs each). Binding/Coverage: share of numeric tokens in the delivered text that is tied to the data. The two are not the same measure: an unresolved SymGen reference renders as \texttt{undefined} and so leaves the text entirely, while an unverifiable ProveML claim stays in it as a marked number. Counting unresolved references as attempted bindings instead gives SymGen 85.0\% and 81.1\%. Resolved is pooled over references, Binding and Coverage are averaged over models. Caught: claims ProveML flagged on the first pass.}
\label{tab:symgen}
\end{table}

\paragraph{On education, substitution binds more and resolves less.} On the educational benchmark SymGen binds a larger share of the numeric output than ProveML (75.4\% vs.\ 67.1\%, or 81.1\% counting unresolved references as attempts), which is unsurprising: emitting a placeholder is easier than transcribing a value correctly. On the compact finance benchmark the two are level (83.6\% vs.\ 85.9\%, or 85.0\% against 85.9\% under the other count) and we draw no conclusion from that row. But 400 of its 1{,}458 references (27\%) address a path that does not exist, and each of those renders as the literal string \texttt{undefined} in the delivered text. Forty-five percent of the educational responses contain at least one. On the compact finance benchmark the same figure is 5\% of references and 10\% of responses.

\paragraph{Both mechanisms fail on addressability; they fail differently.} ProveML is not immune to this: on the same benchmark, 189 of its 1{,}490 first-pass claims (13\%) carry an addressability error, and addressability dominates its residual failures (75--88\%, Section~\ref{sec:education}). Neither approach prevents a model from choosing the wrong path when there are 95 entities to choose between, though substitution does so at more than twice the rate here. The difference is what the reader receives: SymGen's failure is a hole in the sentence, ProveML's is a claim carrying a status and the value that was expected instead. ProveML flagged 229 claims on the first pass of the educational benchmark, 40 of them wrong values --- a class SymGen cannot produce, and equally cannot report.
...
\paragraph{Method.} We ran a deterministic token audit over the stored final responses of Experiments 3 and 4 (all four models, 3 runs each). Coverage is defined as $\textit{marked} / (\textit{marked} + \textit{unmarked})$, where \textit{marked} counts \texttt{\%[field]\{value\}} constructs containing a digit, and \textit{unmarked} counts standalone numeric tokens remaining after stripping all ProveML constructs from the response. Excluded from the unmarked count: list markers, calendar and fiscal years, digits embedded in identifiers (e.g., class codes like \texttt{6ZW}), and code spans. The audit script is included in the research repository (\texttt{experiments/coverage-audit.mjs}). Stored responses are truncated at 2{,}000 characters; this affects mainly Phi-3 on education (36 of 80 responses hit the cap, so that cell is measured on response prefixes), and to a lesser degree Phi-3 on the English prompts (7 of 84) and on finance (2 of 30); no other cell is affected. Table~\ref{tab:coverage} reports coverage per model and benchmark.
...
\midrule
Phi-3 Mini (3.8B) & 41.6\% & 84.6\% \\
Qwen 2.5 3B & 69.8\% & 91.9\% \\
Claude Haiku & 87.3\% & 86.7\% \\
Qwen 2.5 7B & 69.7\% & 80.2\% \\
\bottomrule
\end{tabular}
...
\paragraph{Findings.} Coverage is high but never complete, and it does not track verification rate. It also responds to prompt language: the Dutch/English columns of Table~\ref{tab:langablation} show coverage rising with translation for the two smallest models (Phi-3 41.6\% $\rightarrow$ 68.7\%, Qwen 3B 69.8\% $\rightarrow$ 93.5\%) while Qwen 7B moves the other way (69.7\% $\rightarrow$ 64.0\%), so coverage is not simply a proxy for competence. Most notably, Qwen 7B combines the \textit{highest} verification rate on the educational benchmark (93\%) with roughly 70\% coverage: nearly a third of its numeric tokens are unverifiable prose. The two metrics must be read together; verification rate alone overstates the verified fraction of a report's numeric content. Inspection of the unmarked tokens shows three clusters: (1) \textbf{query-echoed thresholds} (``classes below 60\%''), which should be inference constructs referencing the registry; (2) \textbf{derived values} with no addressable path in the fact store --- counts, differences, and benchmark constants (``a current ratio below 1.0'') --- which the current specification cannot express as facts; and (3) \textbf{store-addressable values left in plain prose}. Token-level classification between these clusters is inherently ambiguous (a bare ``5'' can be a count, an echoed threshold, or a store value), so we do not rank them: what the audit establishes is the size of the unmarked residue, not its composition. Clusters 1 and 3 are prompt-compliance failures that coverage measurement makes visible and a stricter system prompt can target; cluster 2 motivates future work on derived-value claims.
...
\begin{itemize}
    \item ProveML plugin (parser + renderer with lexical scope stack): 515 lines of JavaScript
    \item Threshold registry: 20 thresholds, exercising the full operator vocabulary of Table~\ref{tab:operators}
    \item Test suite: 186 tests (108 plugin, 29 grammar, 15 paper examples, 14 CLI, 20 detection)
