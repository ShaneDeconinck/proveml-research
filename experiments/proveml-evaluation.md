...
Because the current prompt context is not the full fact store, the benchmark avoids prompts that require unseen fields. Offering-level prompts also use unique class names, so no prompt can resolve to more than one offering.
