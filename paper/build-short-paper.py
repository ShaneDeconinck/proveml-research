...
opening         = src[src.index('Large Language Models (LLMs) produce fluent'):src.index('In short:')].rstrip()
intro_ixbrl     = src[src.index('In short: \\textbf{ProveML is iXBRL'):src.index('\\begin{figure}', src.index('In short:'))].rstrip()
...
# the report's middle paragraph restates Finding 1 nearly verbatim; the short
# paper said it three pages ago, so one sentence replaces it
_c_start = conclusion.index('In our evaluation across four models')
_c_end = conclusion.index('What LLMs provide')
conclusion = conclusion[:_c_start] + 'The through-line of the evaluation (Section~\\ref{sec:evaluation}) is that the capability threshold tracks the shape of the request --- schema size, prompt language, context selection --- rather than the size of the model, with the architecture, verifier and grammar unchanged across every condition.\n\n' + conclusion[_c_end:]
...
limitations = limitations.replace('depending on model, benchmark, and prompt language.', 'depending on model and benchmark.')
...
detection = detection.replace('the convergence experiment (Section~\\ref{sec:education})', 'the correction-loop results in Section~\\ref{sec:evaluation}')
...
This paper is deliberately compact: it gives the idea, the three constructs, what the mechanism costs, and what we measured. The complete specification --- verification algorithm, three-valued condition semantics, rendering states, canonicalization rules --- and the full evaluation with all ablations are in the accompanying technical report, published in the artifact repository alongside every benchmark, run artifact, and the scripts that regenerate every table in both documents.\footnote{\url{https://github.com/abovebeyond-ai/proveml-research} --- reference implementation: \url{https://github.com/abovebeyond-ai/proveml} (npm: \texttt{proveml}).}
